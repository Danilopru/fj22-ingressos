package br.com.caelum.ingresso.validação;
...
public class GerenciadorDeSessao {
	...
	private boolean horarioIsValido(Sessao sessaoExistente, Sessao sessaoAtual) {
		...
		if (ehAntes) {
			...
			return horarioAtual.plus(sessaoAtual.getFilme().getDuracao())
	...
	public boolean cabe(Sessao sessaoAtual) {
		...
		Optional<Boolean> optionalCabe = sessoesDaSala
				.stream()
				.map(sessaoExistente -> horarioIsValido(sessaoExistente,
						sessaoAtual)).reduce(Boolean::logicalAnd);
